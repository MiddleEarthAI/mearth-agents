...
export async function getWalletKey(
  ...
  if (teeMode !== TEEMode.OFF) {
    ...
    // Create a new Keypair from the secret key to ensure version compatibility
    const keypair = Keypair.fromSecretKey(deriveKeyResult.keypair.secretKey);
    return requirePrivateKey ? { keypair } : { publicKey: keypair.publicKey };
  }
